...
export default function AdminOverview() {
  ...
  const stats = [
    { label: 'Courses', count: data.courses?.length || 0, to: '/admin/courses' },
    { label: 'Learning paths', count: data.paths?.length || 0, to: '/admin/paths' },
    { label: 'Library items', count: data.resources?.length || 0, to: '/admin/resources' },
    { label: 'Events', count: data.events?.length || 0, to: '/admin/events' },
    { label: 'News posts', count: data.news?.length || 0, to: '/admin/site' },
    { label: 'Learner records', count: data.progress?.completions?.length || 0, to: '/admin/progress' },
  ];
